...
//		glBufferData(GL_PIXEL_PACK_BUFFER, 256 * 192 * sizeof(u32), NULL, GL_STREAM_READ_ARB);
//	}
//	
...
Render3DError OpenGLRenderer_1_2::CreateMultisampledFBO()
{
	...
	GLint maxSamples = 0; //Will Edited, try turning this up
	...
	glRenderbufferStorageMultisampleAPPLE(GL_RENDERBUFFER, maxSamples, GL_RGBA, 256, 192);
	glBindRenderbuffer(GL_RENDERBUFFER, OGLRef.rboMultisampleDepthStencilID);
	glRenderbufferStorageMultisampleAPPLE(GL_RENDERBUFFER, maxSamples, GL_DEPTH24_STENCIL8_OES, 256, 192);
	...
	glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, OGLRef.rboMultisampleDepthStencilID);
	glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER, OGLRef.rboMultisampleDepthStencilID);
...
//	glActiveTextureARB(GL_TEXTURE0 + OGLTextureUnitID_ToonTable);
//	
...
//	
//	glActiveTextureARB(GL_TEXTURE0 + OGLTextureUnitID_ClearImage);
//	
...
//	glBindTexture(GL_TEXTURE_2D, OGLRef.texClearImageDepthStencilID);
//	glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 256, 192, GL_DEPTH_STENCIL, GL_UNSIGNED_INT_24_8, clearImageDepthBuffer);
//	glBindTexture(GL_TEXTURE_2D, 0);
//	
//	glActiveTextureARB(GL_TEXTURE0);
...
//	{
//		glBindVertexArray(OGLRef.vaoMainStatesID);
//		glBufferSubDataARB(GL_ARRAY_BUFFER, 0, sizeof(VERT) * vertList->count, vertList);
//		glBufferSubDataARB(GL_ELEMENT_ARRAY_BUFFER, 0, vertIndexCount * sizeof(GLushort), indexBuffer);
//	}
//	else
//	{
//		if (this->isShaderSupported)
...
//			{
//				glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, OGLRef.iboIndexID);
//				glBufferSubDataARB(GL_ELEMENT_ARRAY_BUFFER, 0, vertIndexCount * sizeof(GLushort), OGLRef.vertIndexBuffer);
//				
//				glBindBuffer(GL_ARRAY_BUFFER, OGLRef.vboVertexID);
//				glBufferSubDataARB(GL_ARRAY_BUFFER, 0, sizeof(VERT) * vertList->count, vertList);
//				glVertexAttribPointer(OGLVertexAttributeID_Position, 4, GL_FLOAT, GL_FALSE, sizeof(VERT), (const GLvoid *)offsetof(VERT, coord));
...
Render3DError OpenGLRenderer_1_2::DownsampleFBO()
{
	...
	glBindFramebuffer(GL_READ_FRAMEBUFFER_APPLE, OGLRef.selectedRenderingFBO);
	glBindFramebuffer(GL_DRAW_FRAMEBUFFER_APPLE, OGLRef.fboFinalOutputID);
...
Render3DError OpenGLRenderer_1_2::ClearUsingImage() const
{
	...
	glBindFramebuffer(GL_READ_FRAMEBUFFER_APPLE, OGLRef.fboClearImageID);
	glBindFramebuffer(GL_DRAW_FRAMEBUFFER_APPLE, OGLRef.selectedRenderingFBO);
...
//		
//		const u32 *__restrict mappedBufferPtr = (u32 *__restrict)glMapBufferARB(GL_PIXEL_PACK_BUFFER, GL_READ_ONLY);
...
Render3DError OpenGLRenderer_1_3::CreateClearImage()
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 256, 192, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL); //Was GL_UNSIGNED_INT_8_8_8_8_REV
...
Render3DError OpenGLRenderer_1_5::CreatePBOs()
{
	...
	{
		glBindBuffer(GL_PIXEL_PACK_BUFFER, OGLRef.pboRenderDataID[i]);
		glBufferData(GL_PIXEL_PACK_BUFFER, 256 * 192 * sizeof(u32), NULL, GL_STREAM_READ);
	}
